...
void main(void)
{
  ...
  int *IX;          /*IX reperesetna un apuntador a un entero.*/
  IX = &X;          /*IX apunta a X. IX tiene la direccion de X.*/
  Y = *IX;          /*Y toma el valor de X de ahora vale 3.*/
  ...
  *IX = *IX - 5;  /*Z[2] se modifica, ahora vale 10. */
  ...
  ++*IX;         /*Z[2] se modica, se incremente en 1. Z[2] ahora vale 11. */
  *IX += 1;      /*Z[2] se vuelve a modificar, ahora vale 12. */
  ...
  es un error que no se senala al compilador del lenguaje C.*/
  printf("\nX = %d \tY = %d \tZ[0] = %d \tZ[1] = %d \tZ[2] = %d \tZ[3] = %d \tZ[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
