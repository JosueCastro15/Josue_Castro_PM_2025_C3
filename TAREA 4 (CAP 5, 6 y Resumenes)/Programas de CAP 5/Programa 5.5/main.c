...
alaumnos, obtiene la frecuencia de cada una de las calificaciones y ademas
...
void main(void)
{
    int CAL[TAM], FRE[6] = {0};   /*Delcaracion de arreglos*/
    Lectura(CAL, TAM);            /*Se llama a la funcion Lectura*/
    Frecuencia(CAL, TAM, FRE, 6);
    /*Se llama a ala funcion Frecuencia se pasan ambos arreglos. */
    printf("\nFrecuencia de calificaciones\n");
    Impresion(FRE, 6);
    Mayor(FRE, 6);
}
...
/*La funcion Lectura se utiliza pra leer el arreglo de calificaciones. */
{
    int I;
    for (I=0; I<T; I++)
    {
        printf("Ingrese la calificacion -0:5- del alaumno %d: ", I+1);
        scanf("%d", &VEC[I]);
    }
    }
void Impresion(int VEC[], int T)
/*La funcion imresion se utiliza para imprimir el arreglo de frecuencias. */
{
    int I;
    for (I=0;I<T;I++)
        printf("\nVEC[%d]: %d", I, VEC[I]);
...
/* Esta funcion calcula la frecuencia de calificaciones. */
{
    ...
    for (I=0; I<P; I++)
        ...
        correcta. */
    B[A[I]]++;  /*Observa la forma de almacenar e incrementar
                las frecuencias*/
}
